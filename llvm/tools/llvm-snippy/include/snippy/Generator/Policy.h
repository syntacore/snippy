...
inline OpcodeFilterType getDefaultFilter(const SnippyTarget &Tgt) {
...
class ModeChangingInstPolicy final : public detail::EmptyFinalizeMixin {
  // We store here opcode filter to use it in other policies
  // (instruction groups) that follow this policy.
  //
  // -- ModeChangingInstPolicy --
  // |
  // | Other Policies (some opcodes are filtered out)
  // |
  // -- another ModeChangingInstPolicy --
  // | ...
  //
...
public:
  ...
  bool isSupport() const {
    return checkMetadata(MetadataMark, SnippyMetadata::Support);
  }
  ...
  const OpcodeFilterType &getOpcodeFilter() const {
    assert(OpcodeFilter && "Opcode filter is not yet created. Usually it is "
...
class BurstGenPolicy final : public detail::EmptyFinalizeMixin {
  OpcodeHistogram ModeCompatibleOpcodes;
  std::vector<InstructionRequest> Instructions;
...
public:
  ...
  void print(raw_ostream &OS) const {
    OS << "Burst Generation Policy: { ModeCompatibleOpcodes:";
    for (auto [Opc, Prob] : ModeCompatibleOpcodes.topOpcodes()) {
      OS << formatv(" {0}({1:P})", Opc, Prob);
    }
    OS << " }\n";
  }
...
class GenPolicy final {
  struct Concept : public RTTIExtends<Concept, RTTIRoot> {
    ...
    // LLVM RTTI
    static inline char ID;
  };

  template <typename PolicyTy>
  class Model final : public RTTIExtends<Model<PolicyTy>, Concept> {
    PolicyTy Data;
  ...
  public:
    ...
    const PolicyTy *getDataPtr() const { return &Data; }
...
public:
  ...
  // Same as llvm::dyn_cast. Returns nullptr if the cast fails.
  template <typename PolicyTy> const PolicyTy *as() const {
    assert(Impl.get());
    if (auto *Derived = dyn_cast<Model<PolicyTy>>(Impl.get()))
      return Derived->getDataPtr();
    return nullptr;
  }
