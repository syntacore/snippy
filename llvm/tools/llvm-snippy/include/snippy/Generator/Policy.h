...
///
/// 5.
/// const ModeChangingInstPolicy *getModeChangingPolicy(MyPolicy &);
/// 6.
/// void setModeChangingPolicy(MyPolicy &, const ModeChangingInstPolicy *);
///
/// The "getModeChangingPolicy" and "setModeChangingPolicy" functions are useful
/// when dealing with context/configuration defining instructions generated by
/// some policy, which can affect several other policies.
///
/// "getModeChangingPolicy" returns pointer to the "ModeChangingInstPolicy" that
/// defines the context for the current policy. If your policy guarantees that
/// it's never affected by any other policy, it can return nullptr.
///
/// "setModeChangingPolicy" allows to change the context-defining policy for the
/// current policy. If your policy is not affected by any other policy, it can
...
public:
  ...
  const std::function<bool(unsigned)> &getOpcodeFilter() const {
...
public:
  ...
  void print(raw_ostream &OS) const {
    OS << "Burst Generation Policy: { Opcodes:";
...
template <typename PolicyTy>
const ModeChangingInstPolicy *getModeChangingPolicy(PolicyTy &Policy) {
  return Policy.getModeChangingPolicy();
}

template <typename PolicyTy>
void setModeChangingPolicy(PolicyTy &Policy,
                           const ModeChangingInstPolicy *ModeChangingPolicy) {
  Policy.setModeChangingPolicy(ModeChangingPolicy);
}
...
class GenPolicy final {
  struct Concept : public RTTIExtends<Concept, RTTIRoot> {
    ...
    virtual const ModeChangingInstPolicy *getModeChangingPolicy() const = 0;
    virtual void
    setModeChangingPolicy(const ModeChangingInstPolicy *ModeChangingPolicy) = 0;
  ...
  public:
    ...
    const ModeChangingInstPolicy *getModeChangingPolicy() const override {
      return planning::getModeChangingPolicy(Data);
    }

    void setModeChangingPolicy(
        const ModeChangingInstPolicy *ModeChangingPolicy) override {
      planning::setModeChangingPolicy(Data, ModeChangingPolicy);
    }
...
public:
  ...
  friend const ModeChangingInstPolicy *
  getModeChangingPolicy(const GenPolicy &Pol);

  friend void
  setModeChangingPolicy(GenPolicy &Pol,
                        const ModeChangingInstPolicy *ModeChangingPolicy);
...
inline const ModeChangingInstPolicy *
getModeChangingPolicy(const GenPolicy &Pol) {
  return Pol.Impl->getModeChangingPolicy();
}

inline void
setModeChangingPolicy(GenPolicy &Pol,
                      const ModeChangingInstPolicy *ModeChangingPolicy) {
  return Pol.Impl->setModeChangingPolicy(ModeChangingPolicy);
}
